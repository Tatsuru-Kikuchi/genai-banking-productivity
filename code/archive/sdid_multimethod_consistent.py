...
Usage: python code/sdid_multimethod_consistent.py
"""
...
class SDID_Bayesian(SDIDBase):
    ...
    def fit(self, n_iter=5000, burnin=1000, thin=2):
        ...
        for i in range(n_iter):
            # Sample tau (Metropolis-Hastings)
            tau_proposal = tau_current + np.random.normal(0, 0.1)
            ...
            shape = 2 + n_eff / 2
            scale = 1 + 0.5 * n_eff * ((sdid_diff - tau_current)**2)
        ...
        if n_intervals > 0:
            ...
            self.hdi_lower = np.percentile(self.tau_samples, 2.5)
            self.hdi_upper = np.percentile(self.tau_samples, 97.5)
    ...
    def plot_posterior(self, output_path=None):
        ...
        ax1 = axes[0]
        ax1.hist(self.tau_samples, bins=50, density=True, alpha=0.7, 
                color='steelblue', edgecolor='black')
        ax1.axvline(self.tau_mean, color='red', linestyle='--', linewidth=2,
                   label=f'Mean = {self.tau_mean:.4f}')
        ...
        ax2 = axes[1]
        ax2.plot(self.tau_samples, alpha=0.5, linewidth=0.5)
...
class SDIDMultiMethod:
    ...
    def fit_all(self, n_bootstrap=500, n_mcmc=5000):
        ...
        if mle.fit(n_bootstrap=n_bootstrap):
            ...
            print(f"    τ = {mle.tau:.4f} (SE = {mle.se:.4f}) {self.results['MLE']['stars']}")
        ...
        if qmle.fit(n_bootstrap=n_bootstrap):
            ...
            print(f"    τ = {qmle.tau:.4f} (SE = {qmle.se:.4f}) {self.results['Q-MLE']['stars']}")
        ...
        if bayes.fit(n_iter=n_mcmc, burnin=1000):
            ...
            self.bayes = bayes
            print(f"    τ = {bayes.tau_mean:.4f} (SD = {bayes.tau_sd:.4f}) P(τ>0)={bayes.prob_positive:.2f}")
...
def main():
    ...
    if all_results:
        ...
        print()
        header = f"{'Outcome':<8}{'Sample':<25}{'Method':<10}{'N_tr':>6}{'N_co':>6}{'τ (ATT)':>12}{'SE':>10}{'95% CI':>24}{'Sig':>12}"
        print(header)
        print("-" * 130)
        ...
        for _, row in summary_df.iterrows():
            ...
            print(f"{row['Outcome']:<8}{row['Sample']:<25}{row['Method']:<10}"
                  ...
                  f"{row['tau']:>12.4f}{row['se']:>10.4f}{ci:>24}{str(row['stars']):>12}")
        
        print("-" * 130)
        print("*** p<0.01, ** p<0.05, * p<0.10 | For Bayesian: P(τ>0) shown")
        print("=" * 130)
        
        # Save
        output_csv = os.path.join(project_root, "data", "processed", "sdid_multimethod_consistent.csv")
        ...
        # =====================================================================
        print("\n" + "=" * 80)
        print("COMPARISON: SDID vs DSDM")
        print("=" * 80)
        print("""
        If SDID and DSDM show similar β estimates:
        → Your results are ROBUST to estimation method
        → The effect is likely CAUSAL (not just correlation)
        
        Key differences:
        - DSDM captures spatial SPILLOVERS (network effects)
        - SDID provides CAUSAL identification (counterfactual)
        
        Ideal finding: Both methods show similar direct effects,
        with DSDM additionally revealing spillover dynamics.
        """)
